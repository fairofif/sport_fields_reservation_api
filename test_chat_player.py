...
def test_create_chat_match_unposted():
    ...
    # clean unittest data
    id_chat_match = response.get_json()['data']['id_chat_match']
    delete_chat_match(id_chat_match)
...
def test_create_chat_match_posted():
    ...
    assert response.status_code == 200
    assert response.get_json()['status'] == True
    assert response.get_json()['message'] == 'Match had been created before, return the old one success'
    assert response.get_json()['data']['id_chat_match'] == id_chat_match
